...
export function generateButtonHoleDieline({ L, W, H, glueFlapWidth, windowDecals = [] }) {
  ...
  const nW = Number(W);
  const nH = Number(H);
  const nGlue = Number(glueFlapWidth) || (nL < 10 ? 16.0 / 25.4 : 16.0);

  // Original parameters from 3_button_hole_box.svg template
  const origL = 75.0;
  const origW = 75.0;
  const origH = 60.0;

  const origYTop = 92.75;
  const origYBot = 152.25;

  const yTop = nW * 1.25;
  const yBot = yTop + nH;

  const origX1 = 16.0;
  const origX2 = origX1 + origL; // 91.0
  const origX3 = origX2 + origW; // 166.0
  const origX4 = origX3 + origL; // 241.0
  const origX5 = 315.5;          // 241.0 + 74.5
  ...
  function mapX(x) {
    if (x <= origX1) {
      ...
      return x4 + ((x - origX4) / (origX5 - origX4)) * nW;
    }
  }
    ...
    while (i < tokens.length) {
      ...
      if (/^[a-zA-Z]$/.test(token)) {
        ...
        if (cmd === 'M' || cmd === 'L' || cmd === 'T') {
          ...
          const x2 = mapX(parseFloat(tokens[i+2]));
          const y2 = mapY(parseFloat(tokens[i+3]));
          result += x1.toFixed(2) + ',' + y1.toFixed(2) + ' ' + x2.toFixed(2) + ',' + y2.toFixed(2) + ' ';
          i += 4;
        } else if (cmd === 'A') {
  ...
  const cutPaths = transformItems(rawButtonHolePaths.cuts);
  const bleedPaths = transformItems(rawButtonHolePaths.bleeds || []);
  
  let foldLines = (rawButtonHolePaths.folds || []).map(item => {
    if (item.type === 'line') {
  ...
  if (windowDecals && windowDecals.length > 0) {
    windowDecals.forEach((windowDecal) => {
      ...
      if (windowDecal.shapeType === "circle") {
        ...
        path = `M ${cx},${cy - size * 0.2} C ${cx + size*0.5},${cy - size*0.8} ${cx + size},${cy - size*0.5} ${cx + size},${cy} C ${cx + size},${cy + size*1.5} ${cx},${cy + size*1.5} C ${cx},${cy + size*1.5} ${cx - size},${cy + size*1.5} ${cx - size},${cy} C ${cx - size},${cy - size*0.5} ${cx - size*0.5},${cy - size*0.8} ${cx},${cy - size * 0.2} Z`;
      } else {
  ...
  const bottomFlapExtension = ((208.75 - origYBot) / origW) * nW;
  const totalHeight = yBot + bottomFlapExtension + (0.1 * nW);
