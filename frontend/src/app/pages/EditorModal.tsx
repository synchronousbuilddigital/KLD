...
export default function EditorModal({ isOpen, onClose, contextType = "mockup" }: { isOpen: boolean; onClose: () => void; contextType?: string }) {
  ...
  const handleExport = async (fileType, colorMode, format) => {
    ...
    // Log export event into backend database history
    try {
      await exportService.logExport({
        format: format === 'AI' ? 'PDF' : 'PDF',
        resolution: 'Vector',
        fileName: filename,
      });
    } catch (logErr) {
      console.warn('Failed to log export event:', logErr);
    }
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (res.success && res.data?.url) {
        ...
        setGalleryImages(prev => [...prev, cdnUrl]);
        handleAddDecal(cdnUrl);
      }
    } catch (err) {
