  ...
  return (
    <>
      <header className="main-header border-b border-zinc-200/80 bg-white/95 backdrop-blur-md transition-all duration-200">
        ...
        <nav className="nav-links flex items-center gap-2 bg-zinc-100/70 p-1.5 rounded-full border border-zinc-200/60">
          {navItems.map((item) => {
            ...
            return (
              <a
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all duration-150 ${
                  isActive 
                    ? 'bg-white text-zinc-950 shadow-sm font-bold border border-zinc-200/80' 
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            className="btn btn-header group bg-zinc-900 hover:bg-zinc-800 text-white text-xs font-bold px-5 py-2.5 rounded-full shadow-sm hover:shadow-md transition-all flex items-center gap-2"
          ...
          <button 
            ...
            className={`px-4 py-2 rounded-full text-xs font-bold flex items-center gap-2 transition-all border shadow-sm ${
              activeNav === 'workspace'
          ...
          {isLoggedIn && (
            <div className="group relative cursor-pointer ml-1">
              <div 
                ...
                className="w-9 h-9 rounded-full bg-zinc-900 text-white flex items-center justify-center font-extrabold text-sm uppercase shadow-sm border border-zinc-800"
                title={currentUser?.role === 'ADMIN' ? 'Admin Control Center' : 'View Profile'}
              >
                {currentUser?.fullName?.[0] || currentUser?.email?.[0] || 'A'}
              </div>
              <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 border border-zinc-200 overflow-hidden">
