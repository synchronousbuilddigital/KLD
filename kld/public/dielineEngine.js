// WARNING: This file is a TEMPORARY legacy compatibility copy for standalone HTML editors in public/.
// The single AUTHORITATIVE source of truth for dieline geometry is:
//   src/geometry/dielineEngine.js
// Any changes to box calculations must be made there. This copy will be removed when the HTML editors are migrated to React.

(function() {
  const Engine = {
    // Top tuck flap path (Right-to-Left tracing)
    getTopTuckPath: function(xStart, L, y2, flapH, tuck) {
      const fw = L * 0.96;
      const ox = (L - fw) / 2;
      const fw2 = fw * 0.99;
      const ox2 = (L - fw2) / 2;
    ...
    getTopDustPath: function(xStart, W, y2) {
      const fw = W * 0.96;
      const ox = (W - fw) / 2;
      const dh = W * 0.5;
      const di = fw * 0.075;
    ...
    getBottomTuckPath: function(xStart, L, y3, flapH, tuck) {
      const fw = L * 0.96;
      const ox = (L - fw) / 2;
      const fw2 = fw * 0.99;
      const ox2 = (L - fw2) / 2;
    ...
    getBottomDustPath: function(xStart, W, y3) {
      const fw = W * 0.96;
      const ox = (W - fw) / 2;
      const dh = W * 0.5;
      const di = fw * 0.075;
    ...
    getStraightTuckGeometry: function(L, W, H, glueTab, tuck, flapH) {
      const g = glueTab;
      ...
      const cutPathCommands = [
        ...
        `L ${x1},${y3}`,
        `L ${x2},${y3}`,
        this.getBottomDustPath(x2, W, y3),
        this.getBottomTuckPath(x3, L, y3, flapH, tuck),
        this.getBottomDustPath(x4, W, y3),
        `L ${x5},${y2}`,
        this.getTopDustPath(x4, W, y2),
        this.getTopTuckPath(x3, L, y2, flapH, tuck),
        this.getTopDustPath(x2, W, y2),
        `L ${x1},${y2}`,
        `L ${x1},${y2 + tc}`,
        `Q ${x1},${y2} ${x1 - tc},${y2}`,
        `L ${x0 + tc},${y2}`,
    ...
    getReverseTuckGeometry: function(L, W, H, glueTab, tuck, flapH) {
      const g = glueTab;
      const x0 = 0, x1 = g, x2 = x1 + L, x3 = x2 + W, x4 = x3 + L, x5 = x4 + W;
      const y0 = 0, y1 = tuck, y2 = y1 + flapH, y3 = y2 + H, y4 = y3 + flapH, y5 = y4 + tuck;
      ...
      const cutPathCommands = [
        ...
        `L ${x3},${y2}`,
        this.getTopDustPath(x2, W, y2),
        this.getTopTuckPath(x1, L, y2, flapH, tuck),
        `L ${x1},${y2}`,
        `L ${x1},${y2 + tc}`,
        `Q ${x1},${y2} ${x1 - tc},${y2}`,
        `L ${x0 + tc},${y2}`,
    ...
    getAutoLockBottomGeometry: function(L, W, H, glueTab) {
      const g = glueTab;
      const tuck = 18;
      const flapH = W;
    ...
    getRscGeometry: function(L, W, H, glueTab) {
      const g = glueTab;
      const flapH = W / 2;
