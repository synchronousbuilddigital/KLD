...
const containerVariants = {
  ...
  open: { rotateX: 60, rotateZ: 30, scale: 0.6, y: 20, transition: smooth },
...
const lidVariants = {
  initial: { rotateX: -90, ...boxStyle, transition: smooth }, // Closed
  open: { rotateX: -40, ...boxStyle, transition: smooth }, // Open wide
  flat: { rotateX: 0, ...boxStyle, transition: smooth }, // Flat
};

const lipVariants = {
  initial: { rotateX: -90, ...boxStyle, transition: smooth }, // Tucked down
  open: { rotateX: -90, ...boxStyle, transition: smooth }, // Stays perpendicular to lid
  flat: { rotateX: 0, ...boxStyle, transition: smooth }, // Flattens out
};
...
export default function MagneticRigidBoxAnimation({ isHovered }: Props) {
    ...
    <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none" style={{ perspective: '1200px' }}>
      <motion.div style={{ width: W, height: D, transformStyle: 'preserve-3d' }} variants={containerVariants} initial="initial" animate={controls}>
        <motion.div style={{ position: 'absolute', inset: 0, ...boxStyle, transformStyle: 'preserve-3d' }}>
          ...
          <motion.div style={{ position: 'absolute', top: '100%', left: 0, width: W, height: H, transformOrigin: 'top', transformStyle: 'preserve-3d' }} variants={frontVariants} />
          ...
          <motion.div style={{ position: 'absolute', bottom: '100%', left: 0, width: W, height: H, transformOrigin: 'bottom', transformStyle: 'preserve-3d' }} variants={backVariants}>
            <motion.div style={{ position: 'absolute', bottom: '100%', left: 0, width: W, height: D, transformOrigin: 'bottom', transformStyle: 'preserve-3d' }} variants={lidVariants}>
              <motion.div style={{ position: 'absolute', bottom: '100%', left: 0, width: W, height: H, transformOrigin: 'bottom', transformStyle: 'preserve-3d' }} variants={lipVariants} />
            </motion.div>
          </motion.div>
